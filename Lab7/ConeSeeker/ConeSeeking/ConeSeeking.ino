...
char model[] = "Lab 7 Cone Seeking";  // Change to your app name.
char versionStr[] = "1.0";
long golfBallOutput = 0;
char outputString[] = "123";
char lab6rxbuf[255];
...
char rightButtonScript[] = "starboard_ball";
...
void detectColor(){
  ...
  golfBallOutput = golfBallOutput + printBallColor(ballColor_1) * 100;
  delay(1000);
  ballColor_2 = stand.determineBallColor(LOCATION_2);
  Serial.print("  Location 2 ball   = ");
  golfBallOutput = golfBallOutput + printBallColor(ballColor_2) * 10;
  delay(1000);
  ballColor_3 = stand.determineBallColor(LOCATION_3);
  Serial.print("  Location 3 ball   = ");
  golfBallOutput = golfBallOutput + printBallColor(ballColor_3) * 1;
  delay(1000);
  stand.setLedState(LED_GREEN, LOCATION_3, LED_FRONT);
  Serial.print("---------------------------------\n\n");
  
  String(golfBallOutput).toCharArray(outputString, 4);
  acc.write(outputString, sizeof(outputString));
...
int printBallColor(int ballColor) {
  switch (ballColor) {
    case BALL_NONE:
